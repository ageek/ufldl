...
% Use Linear decoder, instead of Sigmoid
a3 = z3;

rho = sparsityParam;
rho_hat = (1/m) * sum(a2,2);
cost = (1/(2*m)) * sumsqr(a3 - data) + ...												% un-regularized cost
		beta * sum(rho*log(rho./rho_hat)+(1-rho)*log((1-rho)./(1-rho_hat))) + ...		% sparse penalty 
		(lambda/2) * ( sumsqr(W1) + sumsqr(W2) ) ;										% regularization

% calculate KL-divergence term to be added to delta2
kl_delta = beta * ( -(rho ./ rho_hat) + (1-rho) ./ (1-rho_hat))	;

%delta3 = -(data-a3) .* (a3.*(1-a3));	% older delta3 for sparseAE
% new delta3 for LinearDecoder , here f(z) is unit function, so f'(z3) = 1
delta3 = -(data - a3) ;
delta2 =  ((W2' * delta3) + repmat(kl_delta, 1, m)) .* (a2.*(1-a2));
